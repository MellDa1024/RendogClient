...
object BackgroundScope : CoroutineScope by CoroutineScope(newFixedThreadPoolContext(2, "Rendog Background")) {
        ...
        return launch {
            while (isActive) {
                try {
                    ...
                    RendogMod.LOG.warn("Error occurred while running background job ${job.name}", e)
                }
                delay(job.delay())
            }
        }
