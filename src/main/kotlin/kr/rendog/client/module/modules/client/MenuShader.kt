...
import kr.rendog.client.commons.extension.previous
...
object MenuShader : Module(
    ...
    private val shader = setting("Shader", ShaderEnum.SPACE, { mode == Mode.SET })
    ...
    private class ShaderProgram(shader: ShaderEnum) {
        ...
        init {
            ...
            if (linked == 0) {
                glDeleteProgram(id)
                RendogMod.LOG.error(glGetProgramInfoLog(id, 1024))
        ...
        private fun createShader(path: String, shaderType: Int): Int {
            ...
            if (compiled == 0) {
                glDeleteShader(id)
                RendogMod.LOG.error(glGetShaderInfoLog(id, 1024))
