...
import kr.rendog.client.event.events.WaypointUpdateEvent
...
import kr.rendog.client.util.math.CoordinateConverter
import kr.rendog.client.util.math.CoordinateConverter.asString
import kr.rendog.client.util.math.VectorUtils.toBlockPos
...
object WaypointManager : Manager {
    ...
    private val file = File(FolderUtils.rendogFolder + "waypoints.json")
    ...
    fun loadWaypoints(): Boolean {
        ...
        val success = try {
            ...
            RendogMod.LOG.info("Waypoint loaded")
            true
        } catch (e: Exception) {
            RendogMod.LOG.warn("Failed loading waypoints", e)
            false
        }

        RendogEventBus.post(WaypointUpdateEvent(WaypointUpdateEvent.Type.CLEAR, null))
        return success
    }
    ...
    fun saveWaypoints(): Boolean {
        return try {
            ...
            RendogMod.LOG.info("Waypoint saved")
            true
        } catch (e: Exception) {
            RendogMod.LOG.warn("Failed saving waypoint", e)
            false
        }
    }
    ...
    fun get(id: Int): Waypoint? {
        ...
        RendogEventBus.post(WaypointUpdateEvent(WaypointUpdateEvent.Type.GET, waypoint))
        return waypoint
    }
    ...
    fun add(locationName: String): Waypoint {
        ...
        return if (pos != null) {
            ...
            RendogEventBus.post(WaypointUpdateEvent(WaypointUpdateEvent.Type.ADD, waypoint))
            waypoint
        } else {
            RendogMod.LOG.error("Error during waypoint adding")
    ...
    fun remove(pos: BlockPos, currentDimension: Boolean = false): Boolean {
        return get(pos, currentDimension)?.let { waypoint ->
            RendogEventBus.post(WaypointUpdateEvent(WaypointUpdateEvent.Type.REMOVE, waypoint))
