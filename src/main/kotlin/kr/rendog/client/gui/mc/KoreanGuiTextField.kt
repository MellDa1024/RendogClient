...
import kr.rendog.client.module.modules.chat.KoreanChat.Language
import kr.rendog.client.module.modules.chat.KoreanChat.language
import kr.rendog.client.module.modules.chat.KoreanChat.ctrlComboBypass
import kr.rendog.client.module.modules.chat.KoreanChat.debugging
...
class KoreanGuiTextField(
        ...
        if (language == Language.KOREAN && textToWrite.length == 1) {
            ...
            if (index != -1) {
                val convertedKoreanChar = korIndex.getOrNull(index) ?: kotlin.run {
                    if (debugging) RendogMod.LOG.info(
                        "TextToWrite : $textToWrite / index : $index / engIndex : $engIndex / korIndex : $korIndex"
                    )
                    return
                }
                try {
                    if (debugging) RendogMod.LOG.info("--------------Typing--------------")
                    if (selectionEnd != this.cursorPosition) removeSelection()
                    if (debugging) RendogMod.LOG.info("FullText : $text - 0 : $convertStartingIndex : $cursorPosition >> " +
    ...
    private fun removeSelection() {
        if (debugging) RendogMod.LOG.info("--------------Removing Selection--------------")
        ...
        if (debugging) RendogMod.LOG.info("$text - $selStart : $selEnd >> ${text.getOrNull(selStart)}, ${text.getOrNull(selEnd)}")
        if (debugging) RendogMod.LOG.info("${text.substring(0 until selStart)} / ${text.substring(selEnd)}")
        if (debugging) RendogMod.LOG.info(text.removeRange(selStart..selEnd))
    ...
    private fun deleteFromCursorWhenKorean() {
        try {
            ...
            if (debugging) RendogMod.LOG.info("--------------Deletion--------------")
            if (debugging) RendogMod.LOG.info("FullText : $text / Len : ${text.length} - 0 : $cursorPosition >> " +
    ...
    private fun resetConvertStartingIndex(num: Int = 0) {
        if (debugging) RendogMod.LOG.info("Forced-cursor move, text.length : ${text.length}, cursor : ${this.cursorPosition}")
