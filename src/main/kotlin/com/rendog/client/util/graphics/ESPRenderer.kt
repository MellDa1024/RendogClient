package com.rendog.client.util.graphics

import com.rendog.client.util.EntityUtils
import com.rendog.client.util.EntityUtils.getInterpolatedAmount
...
class ESPRenderer {
    ...
    fun add(entity: Entity, color: ColorHolder, sides: Int) {
        val interpolatedBox = entity.renderBoundingBox.offset(getInterpolatedAmount(entity, RendogTessellator.pTicks()))
    ...
    fun render(clear: Boolean, cull: Boolean = true) {
        ...
        val interpolatedPos = EntityUtils.getInterpolatedPos(entity, RendogTessellator.pTicks())
    ...
    private fun drawList(type: Type, cull: Boolean = false) {
        RendogTessellator.begin(if (type == Type.FILLED) GL_QUADS else GL_LINES)
    ...
    private fun drawFilled(cull: Boolean, box: AxisAlignedBB, color: ColorHolder, sides: Int) {
        ...
        if (!cull || frustumCamera.isBoundingBoxInFrustum(box)) {
            RendogTessellator.drawBox(box, color, a, sides)
        }
    }
    ...
    private fun drawOutline(cull: Boolean, box: AxisAlignedBB, color: ColorHolder, sides: Int) {
        ...
        if (!cull || frustumCamera.isBoundingBoxInFrustum(box)) {
            RendogTessellator.drawOutline(box, color, a, side, thickness)
        }
    }
    ...
    private fun drawTracer(box: AxisAlignedBB, color: ColorHolder) {
        ...
        RendogTessellator.drawLineTo(offsetBox, color, a, thickness)
    }
